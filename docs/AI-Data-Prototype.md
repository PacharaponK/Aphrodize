...
`scripts/wrinkle_prototype.py` loads an RGB face image and a grayscale wrinkle mask, resizes image/mask together to 512×512 (bilinear / nearest-neighbour), thresholds the mask at 127, records simple image-quality flags, and calculates a deterministic whole-face score.
...
```powershell
py scripts/wrinkle_prototype.py --image path\to\face.png --mask path\to\wrinkle-mask.png
```

Run up to 10 manual pairs from the expected FFHQ layout:

```powershell
py scripts/wrinkle_prototype.py --data-root storage\data\non_time_serie\ffhq_wrinkle --limit 10
```
